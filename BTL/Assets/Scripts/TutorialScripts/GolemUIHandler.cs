...
public class GolemUIHandler : MonoBehaviour
{
    [SerializeField] GolemController boss;
    ...
    void FindReferencesIfNeeded()
    {
        ...
        {
            boss = FindFirstObjectByType<GolemController>();
        }
    ...
    int GetHealthImageScore(string imageName)
    {
        ...
        if (lowerName.Contains("boss")) score += 80;
        if (lowerName.Contains("health") || lowerName.Contains("hp")) score += 80;
        if (lowerName.Contains("bar") || lowerName.Contains("fill")) score += 100;
        if (lowerName.Contains("back") || lowerName.Contains("background") || lowerName.Contains("bg") || lowerName.Contains("frame")) score -= 200;
    ...
    {
        if (fillImage == null || !useImageFillAmount) return;
    ...
    {
        if (healthFillRect == null || originalFillWidth > 0f) return;
    ...
    {
        if (fillRect == null || Mathf.Approximately(fillRect.pivot.x, 0f)) return;
    ...
    void UpdateHealthBar()
    {
        ...
        // Lấy Max Health và Current Health từ Boss
        int maxHealth = Mathf.Max(1, boss.maxHealth);
    ...
    {
        if (bossCanvas != null) bossCanvas.enabled = visible;
